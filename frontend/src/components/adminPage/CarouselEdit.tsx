...
export default function CarouselEdit({first_link, second_link, third_link, forth_link} : CarouselEditProps) {
	...
	const handleForthChangeInput = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
  ...
  return (
	<div>
		...
		<div className="shadow-md rounded-lg bg-blue-900 mt-2">
			<textarea onChange={handleForthChangeInput} 
			className={`${getInputClass} ${"text-center py-2 border-blue-500 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full "}`}
			value={editForthLink} disabled={edit === true ? true : false} />		
		</div>
		</div>
